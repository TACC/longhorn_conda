...
CCREATE = conda create -yn $(notdir $@) --strict-channel-priority
...
$(PREFIX)/envs/py3_powerai_1.6.2: | $(DEPS) $(DIRS)
	export $(CENV) ENV=powerai_1.6.2 PYV=3; \
	$(CCREATE) powerai=1.6.2 powerai-rapids=1.6.2 python=$${PYV} $(HOROVOD) && \
	source activate $(notdir $@) && \
	HOROVOD_CUDA_HOME=$(OD)/$@ HOROVOD_GPU_ALLREDUCE=DDL pip install horovod --no-cache-dir
...
$(PREFIX)/envs/py2_powerai_1.6.1: | $(DEPS) $(DIRS)
	export $(CENV) ENV=powerai_1.6.1 PYV=2; \
	$(CCREATE) powerai=1.6.1 python=$${PYV} $(HOROVOD) && \
	source activate $(notdir $@) && \
	HOROVOD_CUDA_HOME=$(OD)/$@ HOROVOD_GPU_ALLREDUCE=DDL pip install horovod --no-cache-dir
...
$(PREFIX)/envs/py3_powerai_1.6.1: | $(DEPS) $(DIRS)
	export $(CENV) ENV=powerai_1.6.1 PYV=3; \
	$(CCREATE) powerai=1.6.1 python=$${PYV} $(HOROVOD) && \
	source activate $(notdir $@) && \
	HOROVOD_CUDA_HOME=$(OD)/$@ HOROVOD_GPU_ALLREDUCE=DDL pip install horovod --no-cache-dir
...
$(PREFIX)/envs/py2_powerai_1.6.0: $(DEPS)
	export $(CENV) ENV=powerai_1.6.0 PYV=2; \
	$(CCREATE) powerai=1.6.1 python=$${PYV} $(HOROVOD) && \
	source activate $(notdir $@) && \
	HOROVOD_CUDA_HOME=$(OD)/$@ HOROVOD_GPU_ALLREDUCE=DDL pip install horovod --no-cache-dir
...
$(PREFIX)/envs/py3_powerai_1.6.0: $(DEPS)
	export $(CENV) ENV=powerai_1.6.0 PYV=3; \
	$(CCREATE) powerai=1.6.1 python=$${PYV} $(HOROVOD) && \
	source activate $(notdir $@) && \
	HOROVOD_CUDA_HOME=$(OD)/$@ HOROVOD_GPU_ALLREDUCE=DDL pip install horovod --no-cache-dir
